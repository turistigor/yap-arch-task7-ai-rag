...
Для этого задания RAG-пайплайн запускался с более мощной моделью (gemma4:e2b), не смотря на то, что ранее была выбрана qwen2.5:7b. Дело в том, что для исходно выбранной модели не получилось реализовать Chain-of-Thought. Я попробовал:
- добавить в system-промпт указание, чтобы модель объясняла свои шаги (Zero-shot CoT),
- добавить примеры с рассуждениями (Few-shot CoT),
- изменить настройки модели (число выдаваемых фрагментов, "температуру", тип поиска).
Однако эти меры не привели к желаемому результату. DeepSeek предполагает, что модель слишком маленькая и не способна на CoT.

Далее приведу несколько скриншотов, иллюстрирующих результаты применения техник промтинга в сравнении с их отсутствием.
